...
class Flavor(base.NovaTimestampObject, base.NovaObject,
    ...
    @classmethod
    def _flavor_get_by_flavor_id_from_db(cls, context, flavor_id, read_deleted):
        """Returns a dict describing specific flavor_id."""
        session = db_api.get_api_session()
        result = _flavor_get_query_db(context, session, read_deleted=read_deleted).\
    ...
    def _flavor_destroy_from_db(self, context, name):
        ...
        ## lalit: need to update read_deleted="no"
        session = db_api.get_session()
        with session.begin():
            ref = db_api.model_query(context, api_models.Flavors, session=session,
                              read_deleted="yes").\
                        filter_by(name=name).\
                        first()
            if not ref:
            ...
            db_api.model_query(context, api_models.FlavorExtraSpecs,
                        ...
                        soft_delete()
            db_api.model_query(context, api_models.FlavorTypeProjects,
                        session=session, read_deleted="yes").\
                        filter_by(flavor_id=ref['id']).\
                        soft_delete()
...
class FlavorList(base.ObjectListBase, base.NovaObject):
    ...
    def _flavor_get_all_from_db(cls, context, inactive=False, filters=None,
        ...
        session = db_api.get_api_session()
        query = _flavor_get_query_db(context, session, read_deleted=read_deleted)
            ...
            if filters['is_public'] and context.project_id is not None:
                the_filter.extend([
                    api_models.Flavors.projects.any(
                       ...
                       # deletd is removed for soft_delete
                ])
            if len(the_filter) > 1:
                query = query.filter(or_(*the_filter))
            else:
